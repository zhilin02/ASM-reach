...
\subsection{Solving the reachability problem of $\singletask$-dominating $\IFAMASS$}\label{sec-ifamass-reach}
...
\item $\Delta_{\Mm}$ is defined as follows:
        \begin{itemize}
			...
			\begin{itemize}
				...
				\end{itemize}
				\item if $\phi\models \neg\ctpflag \wedge \rtfflag$ such that $A \neq B$, we have $(p_0, A, BA, \tau_{\not B}, p_0) \in \Delta_{\Mm}$ 
                and $(p_0, A, BA, \tau_{B, \dag}, p_0) \in \Delta_{\Mm}$,
                %
				\item if $\phi \models \neg\ctpflag \wedge \neg \rtfflag \wedge \stpflag$ and $A \neq B$, we have $(p_0, A, BA, \tau_{\id}, p_0) \in \Delta_{\Mm}$,
%
				\item if $\phi \models \neg\ctpflag \wedge \neg \rtfflag \wedge \neg \stpflag$, we have $(p_0, A, BA, \tau_{\id}, p_0) \in \Delta_{\Mm}$.
			\end{itemize}
            %
        \end{itemize}
\end{itemize}
