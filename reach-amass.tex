...
To resolve this problem, we propose a new idea by \emph{focusing on the main task (namely, the task with the main activity $A_0$ as the bottom activity), constructing a {\WOTrPDS} for the main task, and abstracting the contents of the other tasks into the control states of the {\WOTrPDS}}. 
...
The {\WOTrPDS} $\Pp_A = (P_A, \Gamma_A, \TranSet_A, \Delta_A)$, where 
\begin{itemize}
...
\item $\Delta_{A}$ is defined as follows:
	    ...
	    %
	    \item moreover, we consider the transitions $B \xrightarrow{\startactivity(\phi)} C$ of $\Mm$ that keep the $A$-task as the top task, to construct $\Delta_A$,  specifically, we consider the transitions $B \xrightarrow{\startactivity(\phi)} C$ such that $B \in \act_\standard \cup \act_\singletop \cup \{A\}$ and if $\phi \models \ntkflag$, then $C = A$, 
	    ...
	    (Recall that according to the definition of $\singletask$-dominating $\AMASS$, for a transition $B \xrightarrow{\startactivity(\phi)} C$, if $B \in \act_\singleinstance$ or $\phi \models \ntkflag$, then $C \in \act_\singletask \cup \act_\singleinstance$.)
	    %	    	
		%	\item for each transition $B \xrightarrow{\startactivity(\phi)} C$ such that if $B \neq A$ and $\phi \models \ntkflag$, then $C = A$, 
			\begin{itemize}
				\item if $B \in \act_\standard \cup \act_\singletop \cup \{A\}$, $\phi \models \neg \ntkflag$, and $C \in \act_\standard \cup \act_\singletop$, then 
				%
				\begin{itemize}
...
%
				...
				\item if $B \in \act_\standard \cup \act_\singletop$, $\phi \models \ntkflag$, and $C = A$,  then we have $(p_0, B, \varepsilon, \tau_{\id}, p_A) \in \Delta_{A}$, $(p_A, A, A, \tau_{\id}, p_0)  \in \Delta_{A}$, and for each $\gamma \in \Gamma_A \setminus \{A\}$, $(p_A, \gamma, \varepsilon, \tau_{\id}, p_A) \in \Delta_{A}$.
				%
            %
            \end{itemize}
        \end{itemize}
...
We shall use the tuples $(\aname_1 \cdots \aname_k, (\AutB_{1},\cdots,\AutB_{k}))$ such that for each $i \in [k]$, $\Aut_i$ is a $\Pp_{A_i}$-{\WOTrNFA} for some $A_i \in \act_\singletask \cup \act_\singleinstance$ satisfying $\aft(A_i) = \aname_i$, to represent the set of reachable configurations of $\Mm$.  

\begin{definition}[{\WOTrNFA}-representations]
    Given a $k$-ary string relation $R \subseteq (\act^*)^k$, a {\WOTrNFA}-representation of $R$ is $\{(\AutB_{i,1},\cdots,\AutB_{i,k})\}_{i\in[n]}$ such that for every $i \in [n]$ and $j \in [k]$,  $\AutB_{i,j}$ is a {\WOTrNFA} over the alphabet $\act$, moreover, $R = \bigcup \limits_{i \in [n]} \Lang_{\mhcancel{\dag,\triangleleft}}(\AutB_{i, 1}) \times \cdots \Lang_{\mhcancel{\dag,\triangleleft}}(\AutB_{i, k})$.
